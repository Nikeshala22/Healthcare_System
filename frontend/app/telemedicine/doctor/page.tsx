...
export default function DoctorTelemedicinePage() {
  ...
  const loadSessions = async () => {
    try {
      const res = await fetch(`${TELEMEDICINE_URL}/api/telemedicine/doctor`, {
  ...
  const endSession = async (appointmentId: string) => {
    try {
      const res = await fetch(
        `${TELEMEDICINE_URL}/api/telemedicine/${appointmentId}/end`,
        {
          method: "PUT",
          headers: getAuthHeaders(),
        }
                    ...
                    {session.status !== "ended" && (
                      <>
                        <Link
                          ...
                          className="inline-block rounded-xl bg-indigo-600 px-4 py-2 text-white text-sm"
                        >
                          Join Session
                        </Link>

                        <button
                          onClick={() => endSession(session.appointmentId)}
